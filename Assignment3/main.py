...
def tournament(DistanceMatrix, Population):
    ...
    for i in range(0, len(Population), 4):
        ...
        Population[order[0, i + 2:i + 4], :] = NewPop(2)
...
def crossover(DistanceMatrix, parents):
    ...
    amount = np.int_(np.ceil(((len(parents[1])-1)/1.5)* np.random.random_sample() + 2))
...
for i in range(0,300):
    ...
    #print('at START')
    #print(routeDistance(distanceMatrix, returnBest(oldPop, distanceMatrix, 1).ravel()))
    newPop = tournament(distanceMatrix, np.array(oldPop)) #needs to be shuffled at some point
    ...
    #print('?????')
    #print(routeDistance(distanceMatrix, returnBest(oldPop, distanceMatrix, 1).ravel()))
    #test = np.vstack([newPop, returnBest(oldPop, distanceMatrix, numberOfBest)])
    oldPop = np.array(killWeak(np.vstack([newPop, returnBest(oldPop, distanceMatrix, numberOfBest)]), distanceMatrix, numberOfBest)) # we kill off the weakest
    #oldPop = newPop
    #print('at EXIT')
    print(routeDistance(distanceMatrix, returnBest(oldPop, distanceMatrix, 1).ravel()))
#print(routeDistance(distanceMatrix, returnBest(oldPop, distanceMatrix, 1).ravel()))
